    ...
    try {
      ...
      await s3.send(
        new PutObjectCommand({
          Bucket: BUCKET,
          Key: `processed/${imageId}/metadata.json`,
      ...
      // 4. OCR + 説明文を結合してベクトル化
      const combinedText = [analysis.ocrText, analysis.description]
      ...
      await s3.send(
        new PutObjectCommand({
          Bucket: BUCKET,
          Key: `image-vectors/${imageId}.json`,
          Body: JSON.stringify(
            { imageId, filename, s3Key: key, text: combinedText, embedding },
            null,
            2,
          ),
          ContentType: "application/json",
      ...
      console.log(`Done: ${imageId}/${filename}`);
    } catch (err) {
...
2. "description": 画像の内容を詳しく説明（写っているもの、人物、動物、場所、テーマ、色調、雰囲気など。意味検索に役立つキーワードを含めてください）
